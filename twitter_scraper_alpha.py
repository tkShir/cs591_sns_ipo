...
def get_stock_data(api, company_name="Draft Kings", ticker="DKNG", ipo_date='2020-04-24'):
	...
	ipo_prev_day = ipo_dt - datetime.timedelta(days = 1) + datetime.timedelta(hours=4)
	...
	tweets = tweepy.Cursor(api.search_full_archive,environment_name="IPOReception", query=f'{company_name} OR "${ticker}"', fromDate=ipo_prev_day_str, toDate=ipo_mkt_open_str).items()
